...
@section('title', 'Edit Transaction - ' . $cdAccount->cd_ac_name)
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-sm text-gray-600">
                        Edit transaction for CD Account: <span class="font-semibold">{{ $cdAccount->cd_ac_name }} ({{ $cdAccount->cd_ac_no }})</span>
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <form action="{{ route('admin.cd_account.transactions.update', [$cdAccount->id, $transaction->id]) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="credit_type" id="credit_type" required
                            ...
                            <option value="CREDIT" {{ (old('credit_type') ?? $transaction->credit_type) == 'CREDIT' ? 'selected' : '' }}>Credit (+)</option>
                            <option value="DEBIT" {{ (old('credit_type') ?? $transaction->credit_type) == 'DEBIT' ? 'selected' : '' }}>Debit (-)</option>
                        </select>
                        @error('credit_type')
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" id="description" rows="4"
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description') ?? $transaction->description }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="mt-6 bg-yellow-50 rounded-xl border border-yellow-200 p-6">
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-yellow-800">Transaction Information</h3>
                    <div class="mt-2 text-sm text-yellow-700">
                        <p><strong>Transaction ID:</strong> #{{ $transaction->id }}</p>
                        <p><strong>Created:</strong> {{ $transaction->created_at->format('M d, Y h:i A') }}</p>
                        <p><strong>Last Modified:</strong> {{ $transaction->updated_at->format('M d, Y h:i A') }}</p>
                        <p><strong>Current Type:</strong>
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $transaction->credit_type === 'CREDIT' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $transaction->credit_type }}
                            </span>
                        </p>
                        <p><strong>Current Amount:</strong> ₹{{ number_format($transaction->transaction_amount, 2) }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-start">
                <div class="flex-shrink-0">
                    <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-2m-2 0H7m2 0H5"></path>
                    </svg>
                </div>
                <div class="ml-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTransactionTypeStyle() {
        ...
        if (creditTypeSelect.value === 'CREDIT') {
            container.classList.add('credit-highlight');
        } else if (creditTypeSelect.value === 'DEBIT') {
            container.classList.add('debit-highlight');
        }
    }

    // Apply initial styling
    updateTransactionTypeStyle();

    // Add event listener for changes
    creditTypeSelect.addEventListener('change', updateTransactionTypeStyle);
});
</script>
